// ============================================================================
// composite_bloom_blur_v  —  对图集中的每个 mip 执行垂直高斯模糊
// ============================================================================
//
// 此 compute shader 在水平模糊之后运行。
// 对图集中每个 mip（mip0 除外，它保持锐利）进行垂直方向的高斯模糊。
//
...
void main()
{
    ...
    // ── 垂直高斯模糊（texelFetch，clamp 在四边形边界内）──────
    vec3 color = texelFetch({{RT_BLOOM}}, atlasCoord, 0).rgb * gaussianWeights[0];

    for (int i = 1; i < 5; i++)
    {
        int upY   = clamp(atlasCoord.y - i, quadYMin, quadYMax - 1);
        int downY = clamp(atlasCoord.y + i, quadYMin, quadYMax - 1);
        color += texelFetch({{RT_BLOOM}}, ivec2(atlasCoord.x, upY),   0).rgb * gaussianWeights[i];
        color += texelFetch({{RT_BLOOM}}, ivec2(atlasCoord.x, downY), 0).rgb * gaussianWeights[i];
    }
